...
let str = 'thomas is good'
let strResult = str.split("").reverse().join("")
console.log(strResult)

let strUpper = str.split(" ")
let strUpperRes = strUpper.map((str) => {
    return str.charAt(0).toUpperCase() + str.substring(1)
}).join(" ")
console.log(strUpperRes)

let opp1 = [3, 5, 4, 2, 1]
let opp2 = [9, 5, 3, 7, 9]
let sumOfOpp = []

for (let i = 0; i < opp1.length; i++) {
    sumOfOpp.push(opp1[i] + opp2[i])
}
console.log(sumOfOpp)

let arrSort = opp1.sort((a, b) => {
    return b - a
})
console.log(arrSort)

let filterTheArr = [null, undefined, 0, 8, 3, "", false]
let newFilteredArr = []
let resultOfFilterArr = filterTheArr.filter((ele) => {
    if (typeof (ele) === 'number' && ele) {
        newFilteredArr.push(ele)
    }
})
